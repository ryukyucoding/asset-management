...
if [[ -z "${PROJECT_ID:-}" || -z "${REGION:-}" || -z "${ARTIFACT_REPO:-}" || -z "${BACKEND_SERVICE:-}" || -z "${DB_INSTANCE_CONNECTION_NAME:-}" || -z "${REDIS_HOST:-}" || -z "${REDIS_PORT:-}" || -z "${DATABASE_URL:-}" || -z "${GCS_BUCKET_NAME:-}" || -z "${CLOUD_RUN_SA:-}" ]]; then
  cat <<'EOF'
...
gcloud run deploy "${BACKEND_SERVICE}" \
  ...
  --max-instances 20 \
  --concurrency 80 \
  --set-env-vars "NODE_ENV=production,DATABASE_URL=${DATABASE_URL},REDIS_URL=redis://${REDIS_HOST}:${REDIS_PORT},STORAGE_DRIVER=gcs,GCS_BUCKET_NAME=${GCS_BUCKET_NAME}" \
